...
export async function GET(req: Request) {
  ...
  try {
    ...
    const [stats, prevStats] = await Promise.all([
      ...
      fetchGadsStats(token, days, days),
    ]);
    const prevMap: Record<string, any> = {};
    (prevStats.campaigns || []).forEach((c: any) => { prevMap[c.id] = c; });
    stats.campaigns = stats.campaigns.map((c: any) => ({
      ...c,
...
async function fetchGadsStats(token: string, days: number, offsetDays: number): Promise<any> {
  ...
  const overallQuery = `SELECT metrics.cost_micros, metrics.clicks, metrics.conversions, segments.date FROM campaign WHERE segments.date BETWEEN '${fmt(startDate)}' AND '${fmt(endDate)}' AND campaign.status = 'ENABLED'`;
  const campaignQuery = `SELECT campaign.id, campaign.name, metrics.cost_micros, metrics.clicks, metrics.conversions, metrics.impressions FROM campaign WHERE segments.date BETWEEN '${fmt(startDate)}' AND '${fmt(endDate)}' AND campaign.status = 'ENABLED'`;
  const convQuery = `SELECT conversion_action.name, metrics.all_conversions, metrics.all_conversions_value FROM conversion_action WHERE conversion_action.name IN ('crm_webhook_qualified_lead','crm_webhook_converted_lead')`;
  ...
  const dailyData: Record<string, { spend: number; conversions: number }> = {};
  (overallResp[0]?.results || []).forEach((r: any) => {
  ...
  (campaignResp[0]?.results || []).forEach((r: any) => {
  ...
  const campaigns = Object.values(campMap).filter((c: any) => c.spend > 10).sort((a: any, b: any) => b.spend - a.spend).map((c: any) => ({
    id: c.id, name: c.name, spend: Math.round(c.spend), spend_lakh: (c.spend / 100000).toFixed(2),
    clicks: c.clicks, conversions: Math.round(c.conversions * 10) / 10,
  ...
  (convResp[0]?.results || []).forEach((r: any) => {
  ...
  return {
    period_days: days, total_spend: Math.round(totalSpend), total_spend_lakh: (totalSpend / 100000).toFixed(2),
    total_clicks: totalClicks, total_conversions: Math.round(totalConversions), cpl,
    qualified_leads: Math.round(qualifiedLeads), enrolled_leads: Math.round(enrolledLeads),
    cost_per_enrolled: enrolledLeads > 0 ? Math.round(totalSpend / enrolledLeads) : 0,
    campaigns, weekly_cpl: offsetDays === 0 ? buildWeeklyCpl(dailyData) : [], fetched_at: new Date().toISOString(),
  };
}

async function gadsSearch(query: string, customerId: string, mccId: string, devToken: string, token: string) {
  const resp = await fetch(`https://googleads.googleapis.com/v23/customers/${customerId}/googleAds:searchStream`, {
    method: "POST",
...
function buildWeeklyCpl(dailyData: Record<string, { spend: number; conversions: number }>) {
